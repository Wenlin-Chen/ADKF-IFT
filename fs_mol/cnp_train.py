...
def parse_command_line():
    parser = argparse.ArgumentParser(
        description="Train a CNP model on molecules.",
...
def make_trainer_config(args: argparse.Namespace) -> CNPModelTrainerConfig:
    return CNPModelTrainerConfig(
...
def main():
    ...
    out_dir, dataset, aml_run = set_up_train_run(
        f"CNPModel_{config.used_features}", args, torch=True
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_trainer = CNPModelTrainer(config=config).to(device)
