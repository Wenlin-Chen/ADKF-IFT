...
from fs_mol.models.adaptive_dkt_vgp import ADKTVGPModel
from fs_mol.utils.adaptive_dkt_vgp_utils import (
    ADKTVGPModelTrainer,
    evaluate_adktvgp_model,
)
...
def parse_command_line():
    parser = argparse.ArgumentParser(
        description="Test an Adaptive DKT VGP model on molecules.",
...
def test(
    ...
    """
    Same procedure as validation for ADKTVGPModel. Each validation task is used to
    evaluate the model more than once, dependent on number of context sizes and samples.
    """

    return evaluate_adktvgp_model(
        model,
        dataset,
...
def main():
    ...
    out_dir, dataset = set_up_test_run("walltime_ADKTVGPModel", args, torch=True)
