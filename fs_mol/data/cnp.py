...
class MoleculeCNPFeatures(FSMolBatch):
...
@dataclass(frozen=True)
class CNPBatch:
    support_features: MoleculeCNPFeatures
    support_labels: np.ndarray
    support_numeric_labels: np.ndarray
    query_features: MoleculeCNPFeatures
...
def batcher_finalizer_fn(batch_data: Dict[str, Any]) -> Tuple[MoleculeCNPFeatures, np.ndarray]:
...
def task_sample_to_cnp_task_sample(
    task_sample: FSMolTaskSample, batcher: FSMolBatcher[MoleculeCNPFeatures, np.ndarray], filter_numeric_labels: bool
) -> FeaturisedCNPTaskSample:
    ...
    return FeaturisedCNPTaskSample(
...
) -> FSMolBatcher[MoleculeCNPFeatures, np.ndarray]:
    return FSMolBatcher(
        max_num_graphs,
        max_num_nodes,
        max_num_edges,
...
def get_cnp_task_sample_iterable(
    ...
    def path_to_batches_pipeline(paths: List[RichPath], idx: int):
        ...
        for _ in range(num_samples):
            ...
            yield task_sample_to_cnp_task_sample(task_sample, batcher, filter_numeric_labels)
