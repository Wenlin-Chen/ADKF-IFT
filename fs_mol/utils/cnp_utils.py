...
from fs_mol.data.cnp import (
    CNPBatch,
    get_cnp_task_sample_iterable,
    get_cnp_batcher,
    task_sample_to_cnp_task_sample,
)
from fs_mol.models.cnp import CNPModel, CNPModelConfig
...
class CNPModelTrainerConfig(CNPModelConfig):
    batch_size: int = 256
        ...
        # compute metric at test time
            ...
            with torch.no_grad():
                if model.config.use_numeric_labels:
                    batch_preds = mean.detach().cpu().squeeze().numpy()
                    task_labels.append(this_batch_numeric_labels.detach().cpu().squeeze().numpy())
                else:
                    batch_preds = torch.sigmoid(mean).detach().cpu().squeeze().numpy()
                    task_labels.append(this_batch_labels.detach().cpu().squeeze().numpy())
...
def evaluate_cnp_model(
    ...
    batcher = get_cnp_batcher(max_num_graphs=batch_size)

    def test_model_fn(
        task_sample: FSMolTaskSample, temp_out_folder: str, seed: int
    ):
        cnp_task_sample = torchify(
            task_sample_to_cnp_task_sample(task_sample, batcher, model.config.use_numeric_labels), device=model.device
        )

        _, result_metrics = run_on_batches(
            model,
            batches=cnp_task_sample.batches,
            batch_labels=cnp_task_sample.batch_labels,
            batch_numeric_labels=cnp_task_sample.batch_numeric_labels,
            train=False,
        )
        
        if model.config.use_numeric_labels:
            logger.info(
                f"{cnp_task_sample.task_name}:"
                f" {cnp_task_sample.num_support_samples:3d} support samples,"
                f" {cnp_task_sample.num_query_samples:3d} query samples."
...
def validate_by_finetuning_on_tasks(
    ...
    """
    Validation function for CNPModel. Similar to test function;
    ...
    task_results = evaluate_cnp_model(
        model,
        dataset,
...
class CNPModelTrainer(CNPModel):
    def __init__(self, config: CNPModelTrainerConfig):
