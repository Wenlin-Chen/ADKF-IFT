...
def parse_command_line():
    parser = argparse.ArgumentParser(
        description="Train an Adaptive DKT VGP model on molecules.",
...
def make_trainer_config(args: argparse.Namespace) -> ADKTVGPModelTrainerConfig:
    return ADKTVGPModelTrainerConfig(
...
def main():
    ...
    out_dir, dataset, aml_run = set_up_train_run(
        f"ADKTVGPModel_{config.used_features}", args, torch=True
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_trainer = ADKTVGPModelTrainer(config=config).to(device)
