...
def sample_meta_datasets(data, dataset, task, n_shot, n_query):
    ...
    neg_sample = sample_inds(range(0, thresh), n_shot+n_query//2)
    pos_sample = sample_inds(range(thresh, len(data)), n_shot+n_query//2)

    s_list_1 = neg_sample[:n_shot] + pos_sample[:n_shot]
    q_list_1 = neg_sample[n_shot:] + pos_sample[n_shot:]
...
def sample_test_datasets(data, dataset, task, n_shot, n_query, update_step=1):
    ...
    neg_sample = sample_inds(range(0, thresh), n_shot+update_step * n_query//2)
    pos_sample = sample_inds(range(thresh, len(data)), n_shot+update_step * n_query//2)

    s_list = neg_sample[:n_shot] + pos_sample[:n_shot]
    q_sample = neg_sample[n_shot:] + pos_sample[n_shot:]
